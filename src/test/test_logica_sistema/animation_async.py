...
class Animation:
    ...
    async def loop(self):
        ...
        - se stato ANIMATION → play_animation()
        - se ACCESS_DENIED/ALLOWED → mostra testo per 5s quindi torna ANIMATION
        - se BURGLAR_ALARM o FIRE_ALARM → mostra testo finché non cambi stato
        """
        while True:
            st = self.state
            # ANIMAZIONE
            ...
            # ACCESSO CONSENTITO
                ...
                self.oled.display.fill(0)
                self.oled.show_text("Accesso consentito", x=50, y=50)
                await asyncio.sleep(5)
                self.state = Animation.ANIMATION

            # ALLARME ANTIFURTO
            elif st == Animation.BURGLAR_ALARM:
                self.oled.display.fill(0)
                self.oled.show_text("ALLARME ANTIFURTO", x=50, y=50)
                # rimane finché lo stato non cambia
                await asyncio.sleep(1)

            # ALLARME INCENDIO
            elif st == Animation.FIRE_ALARM:
                self.oled.display.fill(0)
                self.oled.show_text("ALLARME INCENDIO", x=50, y=50)
                await asyncio.sleep(1)
